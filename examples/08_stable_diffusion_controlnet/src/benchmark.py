...
def benchmark_unet(
    ...
    exe_module = Model(os.path.join(ait_so_path, "UNet2DConditionModelControlNet/test.so"))
    if exe_module is None:
        print("Error!! Cannot find compiled module for UNet2DConditionModelControlNet.")
        exit(-1)
...
def benchmark_clip(
    ...
    exe_module = Model(os.path.join(ait_so_path, "CLIPTextModel/test.so"))
    if exe_module is None:
...
def benchmark_vae(
    ...
    exe_module = Model(os.path.join(ait_so_path, "AutoencoderKL/test.so"))
    if exe_module is None:
