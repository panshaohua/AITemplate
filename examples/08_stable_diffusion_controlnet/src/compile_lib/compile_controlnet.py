...
def compile_controlnet(
    ...
    controlnet_cond_pt_ait = Tensor(
        [batch_size, height*8, width*8, 3], name="input3", is_input=True
    )

    Y = ait_mod(latent_model_input_ait, 
                timesteps_ait, 
                text_embeddings_pt_ait,
                controlnet_cond_pt_ait)
    mark_output(Y)

    target = detect_target(
        use_fp16_acc=use_fp16_acc, convert_conv_to_gemm=convert_conv_to_gemm
    )
    compile_model(Y, target, ait_so_path, "ControlNetModel", constants=params_ait)
